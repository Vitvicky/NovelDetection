...
def distanceCalcu(point1,point2):
    length = len(point1)
    sum = 0.0
    for i in xrange(length):
        sum+=power((point1[i]-point2[i]),2)
...
def computeSigmaRadius(indexInClust, dataSet, centerPoint):
    ...
    # print "mean value of cluster ", meanDis
    # print "var value of cluster ", varDis
    #return sqrt(meanDis) + c * varDis
...
def purityAndLabelOfClus(clusterAssment, k, sourceLabel, clusterID):
    purity = 0.0
    label = 0.0
    # for j in range(k):
    # everyInfo = 0.0
    ...
    total = len(indexInClust) * 1.0
    print "In" + str(clusterID) + " th cluster, the total point is", total
    for i in indexInClust:
    ...
    for key in labelSet:
        if maxPoint < labelSet[key]:
            maxPoint = labelSet[key]
            label = key

    print "maxLabel number", maxPoint
    purity = maxPoint / total * 1.0
...
def nearsetClus(dataIndex, feature, label, centroids, k, clusterAssment):
    # everyCluster = 0.0
    point = feature[dataIndex]
    ...
    for j in range(k):
        distance = euclDistance(point, centroids[j, :])
    ...
    purity, predictLabel = purityAndLabelOfClus(clusterAssment, k, label, assignClusIndex)
    #if predictLabel == label[dataIndex][0]:
    return predictLabel,purity,minDisClu,assignClusIndex
    # else:
    #     return 0,purity,minDisClu,assignClus


def detect(dataPoint, centroids, k, radiusCluster):
    count = 0.0
    # outlierDist = []
